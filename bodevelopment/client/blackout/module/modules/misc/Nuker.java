...
public class Nuker extends Module {
   ...
   private final List<Pair<BlockPos, Double>> instants;
   ...
   public Nuker() {
      ...
      this.started = false;
      this.ended = BlockPos.field_10980;
   }
      ...
      if (BlackOut.mc.field_1724 != null && BlackOut.mc.field_1687 != null) {
         ...
         if (this.instants.isEmpty()) {
            ...
            this.instants.forEach((pair) -> {
               this.updateStartOrAbort((BlockPos)pair.method_15442());
            });
         }
   ...
   private BlockPos findPos() {
      this.instants.clear();
      BlockPos middle = BlockPos.method_49638(BlackOut.mc.field_1724.method_33571());
      ...
      for(int x = -rad; x <= rad; ++x) {
         for(int y = -rad; y <= rad; ++y) {
            for(int z = -rad; z <= rad; ++z) {
               BlockPos pos = middle.method_10069(x, y, z);
                           ...
                           if (isInstant) {
                              if (this.instants.size() < (Integer)this.maxInstants.get()) {
                                 ...
                                 for(int i = 0; i < this.instants.size(); ++i) {
                                    Pair<BlockPos, Double> pair = (Pair)this.instants.get(i);
      ...
      if (this.instants.size() > 0) {
         return (BlockPos)((Pair)this.instants.get(0)).method_15442();
      } else {
         return bestPos;
      }
   }
   ...
   private double getProgress() {
      if (this.minePos == null) {
         ...
         ItemStack itemStack = this.findBestSlot((stack) -> {
   ...
   private void start(BlockPos pos) {
      ...
      if (dir != null) {
         ...
         if (!holding && !canInstant) {
            ...
            Managers.BLOCK.set(pos, Blocks.field_10124, true, true);
         }
   ...
   private Box getBox(double sides, double up, double down) {
      VoxelShape shape = BlackOut.mc.field_1687.method_8320(this.minePos).method_26218(BlackOut.mc.field_1687, this.minePos);
