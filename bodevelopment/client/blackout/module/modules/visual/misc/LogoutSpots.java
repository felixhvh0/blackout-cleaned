...
public class LogoutSpots extends Module {
   ...
   private final TimerMap<UUID, ItemStack[]> prevItems;
   private final TimerList<UUID> removedUUIDs;
   private final TimerList<AbstractClientPlayerEntity> removedEntities;
   ...
   public void onEntityRemove(PacketEvent.Receive.Pre event) {
      ...
      if (var3 instanceof PlayerRemoveS2CPacket) {
         PlayerRemoveS2CPacket packet = (PlayerRemoveS2CPacket)var3;
         ...
         BlackOut.mc.field_1687.method_18456().forEach((player) -> {
            ...
            if (!this.itemsEmpty(player)) {
               ...
               if (this.prevItems.containsKey(uuid)) {
                  stacks = (ItemStack[])this.prevItems.get(uuid);
               } else {
   ...
   private boolean itemsEmpty(AbstractClientPlayerEntity player) {
      for(int i = 0; i < 4; ++i) {
         ItemStack armorStack = player.method_31548().method_7372(3 - i);
   ...
   private void renderInfo(LogoutSpots.Spot spot) {
      ...
      Vec2f coords = RenderUtils.getCoords(pos.method_10216(), pos.method_10214(), pos.method_10215(), true);
      if (coords != null) {
         this.matrixStack.method_22903();
   ...
   private void renderComponent(LogoutSpots.ItemComponent component, int textColor) {
      ...
      if (component.armor()) {
         NbtCompound nbt = itemStack.method_7969();
   ...
   private Vec3d infoPos(LogoutSpots.Spot spot) {
   ...
   private boolean checkMatchingEntities(UUID uuid) {
      return !this.removedEntities.removeAll((timer) -> {
         AbstractClientPlayerEntity player = (AbstractClientPlayerEntity)timer.value;
   ...
   private static class Spot {
      ...
      public Spot(AbstractClientPlayerEntity player) {
         ...
         PlayerListEntry entry = BlackOut.mc.method_1562().method_2871(player.method_7334().getId());
         if (entry != null) {
      ...
      private void fillItems() {
         ...
         if (map.containsKey(uuid)) {
            ItemStack[] arr = (ItemStack[])map.get(uuid);
   ...
   private static record ItemComponent(ItemStack itemStack, boolean armor) {
      private ItemComponent(ItemStack itemStack, boolean armor) {
