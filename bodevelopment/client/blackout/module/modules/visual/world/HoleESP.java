...
public class HoleESP extends Module {
   ...
   private float getHeight(BlockPos pos) {
      ...
      return (float)MathHelper.method_16436(Math.sin(offset / 2.0D + (double)System.currentTimeMillis() * (Double)this.breathingSpeed.get() / 500.0D) / 2.0D + 0.5D, (Double)this.minHeight.get(), (Double)this.maxHeight.get());
   }

   public static void fadePlane(Matrix4f matrix4f, VertexConsumer vertexConsumer, float x1, float z1, float x2, float z2, float x3, float z3, float x4, float z4, float y, float height, float r, float g, float b, float a) {
   ...
   private void hline(VertexConsumer consumer, Matrix4f matrix4f, Matrix3f matrix3f, float x, float z, float x2, float z2, float y, float r, float g, float b, float a) {
      float dx = x2 - x;
      float dz = z2 - z;
   ...
   public void fadeLine(Matrix4f matrix4f, Matrix3f matrix3f, VertexConsumer vertexConsumer, float x, float z, float y, float height, float r, float g, float b, float a) {
   ...
   private float getAlpha(double dist) {
      return (float)MathHelper.method_15350(1.0D - (dist - (Double)this.range.get() / 2.0D) / ((Double)this.range.get() / 2.0D), 0.0D, 1.0D);
   }
