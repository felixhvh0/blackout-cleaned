...
public class Nametags extends Module {
   ...
   private final Setting<List<Enchantment>> enchantments;
   ...
   public void renderNameTag(double tickDelta, Entity entity) {
      ...
      this.stack.method_22903();
      Vec2f f = RenderUtils.getCoords(x, y + entity.method_5829().method_17940() + (Double)this.yOffset.get(), z, true);
      if (f == null) {
         this.stack.method_22909();
      } else {
         ...
         if (entity instanceof AbstractClientPlayerEntity) {
            ...
            if ((Boolean)this.ping.get()) {
               PlayerListEntry entry = BlackOut.mc.method_1562().method_2871(entity.method_5667());
               if (entry != null) {
         ...
         if ((Boolean)this.armor.get() || (Boolean)this.hand.get()) {
            ...
            if (entity instanceof AbstractClientPlayerEntity) {
               AbstractClientPlayerEntity livingEntity = (AbstractClientPlayerEntity)entity;
               ...
               if ((Boolean)this.armor.get()) {
                  for(int i = 0; i < 4; ++i) {
                     ItemStack itemStack = livingEntity.method_31548().method_7372(3 - i);
   ...
   private void drawEnchantments(ItemStack itemStack, float wbg, float separation, int i) {
      Map<Enchantment, Integer> map = EnchantmentHelper.method_8222(itemStack);
      if (!map.isEmpty() && (Boolean)this.drawEnchants.get()) {
         int y = 0;
         Iterator var7 = map.entrySet().iterator();

         while(var7.hasNext()) {
            Entry<Enchantment, Integer> entry = (Entry)var7.next();
   ...
   private Color getColor(LivingEntity entity, float health) {
      return this.colorMode.get() == Nametags.ColorMode.Custom ? ((BlackOutColor)this.hp.get()).getColor() : ColorUtils.lerpColor((double)Math.min(health / (entity.method_6063() + (float)(entity instanceof PlayerEntity ? 16 : 0)), 1.0F), new Color(255, 0, 0, 255), new Color(0, 255, 0, 255));
   }
