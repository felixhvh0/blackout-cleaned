...
public class Trails extends Module {
   ...
   private final Map<Entity, Trails.Line> map;
   private long prevAdd;

   public Trails() {
      super("Trails", "Renders lines behind entities.", SubCategory.ENTITIES, true);
      this.entities = this.sgGeneral.el("Entities", "", EntityType.field_6082);
         ...
         BlackOut.mc.field_1687.method_18112().forEach((entity) -> {
            if (((List)this.entities.get()).contains(entity.method_5864())) {
               if (this.map.containsKey(entity)) {
                  ((Trails.Line)this.map.get(entity)).positions.add(new Pair((Vec3d)((Trails.HeightMode)this.renderHeight.get()).function.apply(entity, tickDelta), System.currentTimeMillis()));
               } else {
                  Trails.Line line = new Trails.Line();
                  line.positions.add(new Pair((Vec3d)((Trails.HeightMode)this.renderHeight.get()).function.apply(entity, tickDelta), System.currentTimeMillis()));
   ...
   public static enum HeightMode {
      ...
      private HeightMode(DoubleFunction<Entity, Double, Vec3d> function) {
      ...
      private void render(MatrixStack stack, Vec3d camPos, Color color, double renderTime, double fadeTime) {
         ...
         if (this.positions.size() >= 2) {
            ...
            for(int i = 0; i < this.positions.size() - 1; ++i) {
               Pair<Vec3d, Long> pair = (Pair)this.positions.get(i);
               Pair<Vec3d, Long> nextPair = (Pair)this.positions.get(i + 1);
               Vec3d vec = (Vec3d)pair.method_15442();
               Vec3d nextVec = (Vec3d)nextPair.method_15442();
               ...
               Vec3d diff = nextVec.method_1020(vec);
               Vec3d normal = diff.method_1021(1.0D / diff.method_1033());
