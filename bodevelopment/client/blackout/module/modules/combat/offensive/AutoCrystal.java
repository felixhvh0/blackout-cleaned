...
public class AutoCrystal extends Module {
   ...
   private final TimerList<Box> spawning;
   private final TickTimerList<BlockPos> existedTicksList;
   private final TimerList<BlockPos> existedList;
   private final TickTimerList<BlockPos> placeDelayTicksList;
   private final TimerList<BlockPos> placeDelayList;
   ...
   private final Map<Entity, Box> boxMap;
   private final List<Box> valueBoxes;
   ...
   private final TimerMap<BlockPos, Integer> hitBoxDesyncList;
   ...
   private final Map<PlayerEntity, Float> moveDirs;
   private final RenderList<BlockPos> earthRender;
   private double renderProgress;
   private BlockPos renderPos;
   private Vec3d renderVec;
   private Vec3d renderTargetVec;
   ...
   private final Predicate<ItemStack> antiWeaknessPredicate;
   public AbstractClientPlayerEntity targetedPlayer;
   ...
   public AutoCrystal() {
      ...
      this.movement = new Vec3d(0.0D, 0.0D, 0.0D);
      this.targets = new ArrayList();
      this.moveDirs = new HashMap();
      this.earthRender = RenderList.getList(false);
      this.renderProgress = 0.0D;
      this.renderPos = BlockPos.field_10980;
      this.renderVec = Vec3d.field_1353;
      this.renderTargetVec = Vec3d.field_1353;
      this.crystalDir = Direction.field_11033;
      ...
      this.antiWeaknessPredicate = (stack) -> {
         return stack.method_7909() instanceof ToolItem;
      };
   ...
   public void onEntity(EntityAddEvent.Pre event) {
      if ((Integer)this.predictAttacks.get() > 0) {
         ...
         if (this.sentId > this.confirmedId) {
            this.sentId = MathHelper.method_15340(this.sentId, this.confirmedId, this.confirmedId + (Integer)this.predictFlexibility.get());
         } else {
      ...
      if (this.enabled) {
         if (event.entity instanceof EndCrystalEntity) {
            BlockPos p = event.entity.method_24515();
            ...
            this.placeDelayList.remove((timer) -> {
               return ((BlockPos)timer.value).equals(p);
            });
            this.placeDelayTicksList.remove((timer) -> {
               return ((BlockPos)timer.value).equals(p);
            });
            this.spawning.remove((timer) -> {
               return BlockPos.method_49638(BoxUtils.feet((Box)timer.value)).equals(event.entity.method_24515());
            });
   ...
   public void onMovePre(MoveEvent.Pre event) {
      ...
      this.moveModifier = MathHelper.method_15350(this.moveModifier, 0.0D, 1.0D);
   }
   ...
   private void debug(String string, String value) {
      ChatUtils.addMessage(string + " " + Formatting.field_1075 + value);
   }
   ...
   private Box expanded(AbstractClientPlayerEntity player, boolean extrapolate, double multi) {
      Vec3d velocity = this.clampVec(player.method_19538(), player.field_6014, player.field_6036, player.field_5969);
      ...
      List<VoxelShape> list = BlackOut.mc.field_1687.method_20743(player, box.method_18804(velocity));
      Vec3d vec = Entity.method_20736(player, velocity.method_18805(multi, 0.0D, multi), box, BlackOut.mc.field_1687, list);
      return box.method_18804(vec);
   }

   private Vec3d clampVec(Vec3d pos, double x, double y, double z) {
      Vec3d vec = pos.method_1023(x, y, z);
   ...
   private void updateMap(ExtrapolationMap extrapolationMap, EpicInterface<Entity, Integer> ticks) {
      ...
      Managers.EXTRAPOLATION.getDataMap().forEach((player, data) -> {
         if (this.targets.contains(player)) {
            Box box = data.extrapolate(player, (Integer)ticks.get(player));
            map.put(player, box);
         }
      });
   }
      ...
      while(true) {
         while(true) {
                  ...
                  do {
                     if (!var2.hasNext()) {
                        ...
                        this.targets.forEach((playerx) -> {
                           Vec3d movement = playerx.method_19538().method_1023(playerx.field_6014, playerx.field_6036, playerx.field_5969);
                           if (!(movement.method_37268() < 0.01D)) {
                              this.moveDirs.put(playerx, (float)RotationUtils.getYaw(Vec3d.field_1353, movement, 0.0D));
                           }
                        });
                        this.updateMaps();
                        return;
            ...
            if (map.size() < (Integer)this.maxTargets.get()) {
               ...
               while(var6.hasNext()) {
                  Entry<PlayerEntity, Double> entry = (Entry)var6.next();
   ...
   private void renderBasic(double delta, boolean disabled) {
      ...
      this.renderProgress = MathHelper.method_15350(this.renderProgress + (renderActive ? delta : -delta), 0.0D, (Double)this.fadeTime.get() + (Double)this.renderTime.get());
      ...
      case Earthhack:
         this.earthRender.update((pos, time, d) -> {
            ...
            this.calcDamage(new Vec3d((double)pos.method_10263() + 0.5D, (double)(pos.method_10264() + 1), (double)pos.method_10260() + 0.5D), false);
      ...
      case BlackOut:
         if (renderActive) {
            ...
            this.renderTargetVec = new Vec3d((double)this.placePos.method_10263() + 0.5D, (double)this.placePos.method_10264() - 0.5D, (double)this.placePos.method_10260() + 0.5D);
         }
         ...
         if (p > 0.0D) {
            ...
            if ((Boolean)this.renderDamage.get()) {
               this.calcDamage(new Vec3d((double)this.renderPos.method_10263() + 0.5D, (double)this.renderPos.method_10264(), (double)this.renderPos.method_10260() + 0.5D), false);
      ...
      case Simple:
         ...
         if (p > 0.0D) {
            ...
            this.calcDamage(new Vec3d((double)this.renderPos.method_10263() + 0.5D, (double)(this.renderPos.method_10264() + 1), (double)this.renderPos.method_10260() + 0.5D), false);
            if ((Boolean)this.renderDamage.get()) {
   ...
   private void moveRender(double delta) {
      ...
      Vec3d offset = this.renderTargetVec.method_1020(this.renderVec);
   ...
   private Box getBox(Vec3d middle, double p) {
      ...
      return new Box(middle.method_10216() - sides, middle.method_10214() - down, middle.method_10215() - sides, middle.method_10216() + sides, middle.method_10214() + up, middle.method_10215() + sides);
   }
   ...
   private void update(boolean canPlace) {
      ...
      if (this.placing) {
         this.calcDamage(new Vec3d((double)this.placePos.method_10263() + 0.5D, (double)this.placePos.method_10264(), (double)this.placePos.method_10260() + 0.5D), false);
   ...
   private boolean canPlace() {
      this.crystalResult = ((AutoCrystal.ACSwitchMode)this.switchMode.get()).find(Items.field_8301);
      if (this.switchMode.get() == AutoCrystal.ACSwitchMode.Gapple && this.gappleSwitch(this.placePos != null)) {
         ...
         this.crystalHand = OLEPOSSUtils.getHand((stack) -> {
            return stack.method_7909() == Items.field_8301;
         });
      ...
      if (!(Boolean)this.attack.get()) {
         ...
         while(true) {
            ...
            do {
               ...
               } while(!this.canAttack(entity, (BlockPos)null));
   ...
   private boolean hasWeakness() {
      return BlackOut.mc.field_1724.method_6088().containsKey(StatusEffects.field_5911);
   }
   ...
   private void attack(int id, Vec3d vec, boolean predict) {
      BlockPos pos = BlockPos.method_49638(vec);
      if (!predict) {
   ...
   private void sendAttack(int id, boolean swing) {
      PlayerInteractEntityC2SPacket packet = PlayerInteractEntityC2SPacket.method_34206(BlackOut.mc.field_1724, BlackOut.mc.field_1724.method_5715());
   ...
   private boolean isBlocked(BlockPos pos) {
      Box box = new Box((double)pos.method_10263(), (double)pos.method_10264(), (double)pos.method_10260(), (double)(pos.method_10263() + 1), (double)(pos.method_10264() + (SettingUtils.cc() ? 1 : 2)), (double)(pos.method_10260() + 1));
      ...
      } while(!((Box)t.value).method_994(box));
               ...
               if (this.speedCheck()) {
                  if (this.placeDelayCheck()) {
                     ...
                     if (this.crystalHand == null && !switched) {
                        ...
                        if ((Integer)this.predictAttacks.get() > 0) {
                           this.sendPredictions(new Vec3d((double)this.placePos.method_10263() + 0.5D, (double)this.placePos.method_10264(), (double)this.placePos.method_10260() + 0.5D));
                        }
   ...
   private boolean gappleSwitch(boolean canPlace) {
      ...
      Item mainHandItem = BlackOut.mc.field_1724.method_6047().method_7909();
      Item offHandItem = BlackOut.mc.field_1724.method_6079().method_7909();
      boolean holdingGapples = mainHandItem == Items.field_8463 || mainHandItem == Items.field_8367;
      boolean holdingCrystals = mainHandItem == Items.field_8301;
      boolean gapplesInOffhand = offHandItem == Items.field_8463 || offHandItem == Items.field_8367;
      boolean crystalsInOffhand = offHandItem == Items.field_8301;
   ...
   private Vec3d getPlaceVec(BlockPos pos) {
      ...
      if ((Boolean)this.moveOffset.get()) {
         x = MathHelper.method_15350(BlackOut.mc.field_1724.method_18798().field_1352, -0.5D, 0.5D);
         z = MathHelper.method_15350(BlackOut.mc.field_1724.method_18798().field_1350, -0.5D, 0.5D);
      }
   ...
   private Vec3d horizontalOffsetVec(double x, double y, double z) {
      double ox = MathHelper.method_15350(this.movement.field_1352, -0.5D, 0.5D);
      double oz = MathHelper.method_15350(this.movement.field_1350, -0.5D, 0.5D);
   ...
   private void updatePos() {
      ...
      Box rangeBox = Managers.EXTRAPOLATION.extrapolate(BlackOut.mc.field_1724, (Integer)this.rangeExt.get());
      if (rangeBox == null) {
         this.rangePos = BlackOut.mc.field_1724.method_33571();
      } else {
         this.rangePos = new Vec3d((rangeBox.field_1323 + rangeBox.field_1320) / 2.0D, rangeBox.field_1322, (rangeBox.field_1321 + rangeBox.field_1324) / 2.0D);
      }

      BlockPos newPos = this.getPlacePos(BlockPos.method_49638(this.rangePos.method_1031(0.0D, (double)BlackOut.mc.field_1724.method_18381(BlackOut.mc.field_1724.method_18376()), 0.0D)), (int)Math.ceil(SettingUtils.maxInteractRange()));
   ...
   private boolean shouldCalc() {
      if (!(Boolean)this.rotationFriendly.get()) {
         ...
         Direction dir = SettingUtils.getPlaceOnDirection(this.placePos.method_10074());
         if (dir == null) {
            return true;
            ...
            for(int y = -rad - 1; y <= rad - 1; ++y) {
               for(int z = -rad; z <= rad; ++z) {
                  ...
                  if (this.crystalBlock(pos)) {
                     Direction dir = SettingUtils.getPlaceOnDirection(pos.method_10074());
                     if (dir != null && (!(Boolean)this.ahd.get() || !this.hitBoxDesyncList.contains((p, timer) -> {
                        ...
                        this.calcDamage(new Vec3d((double)pos.method_10263() + 0.5D, (double)pos.method_10264(), (double)pos.method_10260() + 0.5D), false);
   ...
   private boolean inAttackRangePlacing(BlockPos pos) {
      ...
      case Semi:
         if (this.inAttackRangePlacing(OLEPOSSUtils.getCrystalBox(pos), (Vec3d)null)) {
            return true;
         }
   ...
   private boolean intersects(BlockPos pos) {
      ...
      if (!(Boolean)this.ignoreItems.get() && EntityUtils.intersectsWithSpawningItem(pos)) {
         ...
         do {
            ...
            TimerList.Timer<Box> timer = (TimerList.Timer)var3.next();
            b = (Box)timer.value;
         } while(!b.method_994(box) || pos.equals(BlockPos.method_49638(BoxUtils.feet(b))));
   ...
   private boolean validForIntersects(Entity entity, BlockPos pos) {
      if ((Boolean)this.ignoreItems.get() && entity instanceof ItemEntity) {
         return false;
      } else if (!(Boolean)this.ignoreExp.get() || !(entity instanceof ExperienceOrbEntity) && !(entity instanceof ExperienceBottleEntity)) {
         ...
         return !(entity instanceof PlayerEntity) || !entity.method_7325();
      } else {
         return false;
      }
   }
   ...
   private boolean canAttack(Entity entity, BlockPos placingPos) {
      ...
      if (placing) {
         if (!this.inAttackRangePlacing(box, (Vec3d)null)) {
            return false;
         }
   ...
   private double getAttackValue(EndCrystalEntity crystal) {
      double value = 0.0D;
      Vec3d feet = crystal.method_19538();
   ...
   private double getPlaceValue(BlockPos pos) {
      ...
      value += this.moveMod(new Vec3d((double)pos.method_10263() + 0.5D, (double)(pos.method_10264() + 1), (double)pos.method_10260() + 0.5D));
      value += this.enemyMod();
      value += this.selfMod();
      value += this.friendMod();
   ...
   private double moveMod(Vec3d vec) {
      ...
      for(Iterator var4 = this.moveDirs.entrySet().iterator(); var4.hasNext(); val += v) {
         Entry<PlayerEntity, Float> entry = (Entry)var4.next();
         ...
         v *= valueMulti;
         v *= 1.0D - MathHelper.method_15350(Math.abs(vec.method_10214() - player.method_23318()) - 1.0D, 0.0D, 1.0D);
      }
   ...
   public boolean shouldAutoMineStop(Entity entity) {
      ...
      if (autoMine.enabled && autoMine.started && (Boolean)this.autoMineAttack.get() && !(autoMine.getCurrentProgress() < (Double)this.autoMineAttackProgress.get()) && autoMine.minePos != null && OLEPOSSUtils.solid2(autoMine.minePos)) {
         ...
         case TrapCev:
         case SurroundCev:
            return BlockPos.method_49638(entity.method_19538().method_1031(0.0D, -0.3D, 0.0D)).equals(autoMine.minePos);
         case SurroundMiner:
         case AutoCity:
         case AntiBurrow:
         case Manual:
   ...
   private boolean crystalBlock(BlockPos pos) {
      Block block = this.getState(pos.method_10074()).method_26204();
      if (block != Blocks.field_10540 && block != Blocks.field_9987) {
         return false;
   ...
   private boolean air(BlockPos pos) {
      return this.getState(pos).method_26204() instanceof AirBlock;
   }
   ...
   private Rotation raytraceRotation(BlockPos pos, boolean getBest) {
      Direction placeDir = SettingUtils.getPlaceOnDirection(pos.method_10074());
      if (placeDir == null) {
         return null;
      } else {
         Vec3d vec = SettingUtils.getRotationVec(pos.method_10074(), placeDir, this.getPlaceVec(pos), RotationType.Interact);
         Rotation rotation = SettingUtils.getRotation(vec);
         double minDist = BlackOut.mc.field_1724.method_33571().method_1025(vec);
         ((IRaycastContext)DamageUtils.raycastContext).blackout_Client$set(ShapeType.field_17558, FluidHandling.field_1348, BlackOut.mc.field_1724);
         ...
         for(float p = 90.0F; p >= -90.0F; p -= 10.0F) {
            ...
            if (!(dist < (float)(Integer)this.raytraceAngle.get()) && !(dist > bestDist)) {
               Vec3d pitchPos = RotationUtils.rotationVec((double)rotation.yaw(), (double)p, BlackOut.mc.field_1724.method_33571(), 10.0D);
