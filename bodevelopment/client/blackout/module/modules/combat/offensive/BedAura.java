...
public class BedAura extends Module {
   ...
   private final RenderList<Pair<BlockPos, Direction>> renderList;
   private BlockPos placePos;
   private Direction placeDir;
   private BlockPos explodePos;
   ...
   private final TimerList<BlockPos> explodeTimers;
   private final TimerList<BlockPos> ignoreState;
   private boolean suicide;
   private boolean facePlacing;
   private BlockPos renderPos;
   private Direction renderDir;
   private double renderProgress;
   private final Map<BlockPos, Double[]> damageCache;
   ...
   public void onState(BlockStateEvent event) {
      if (event.state.method_26204() instanceof BedBlock) {
         this.explodeTimers.remove((timer) -> {
            return ((BlockPos)timer.value).equals(event.pos);
         });
   ...
   private void updateRender() {
      ...
      if ((Boolean)this.renderExplode.get()) {
         this.renderList.update((pair, time, delta) -> {
            this.render((BlockPos)pair.method_15442(), (Direction)pair.method_15441(), this.separateExplode, this.explodeHeadLineColor, this.explodeHeadSideColor, this.explodeHeadShape, this.explodeLineColor, this.explodeSideColor, this.explodeRenderShape, this.getAlpha(time, this.explodeRenderTime, this.explodeFadeTime));
         });
      }
   ...
   private void render(BlockPos feetPos, Direction dir, Setting<Boolean> separate, Setting<BlackOutColor> headLines, Setting<BlackOutColor> headSides, Setting<RenderShape> headShape, Setting<BlackOutColor> feetLines, Setting<BlackOutColor> feetSides, Setting<RenderShape> feetShape, double alpha) {
   ...
   private Box getBoxAt(BlockPos pos) {
      return new Box((double)pos.method_10263(), (double)pos.method_10264(), (double)pos.method_10260(), (double)(pos.method_10263() + 1), (double)pos.method_10264() + 0.55D, (double)(pos.method_10260() + 1));
   }
   ...
   private void place() {
      PlaceData data = SettingUtils.getPlaceData(this.placePos, (DoublePredicate)null, (p, d) -> {
         if ((Boolean)this.floor.get() && d != Direction.field_11033) {
            return false;
         } else {
            return !(BlackOut.mc.field_1687.method_8320(p).method_26204() instanceof BedBlock);
         }
      });
      if (data.valid()) {
         ...
         if (hand != null || result.wasFound()) {
            if (!SettingUtils.shouldRotate(RotationType.BlockPlace) || this.rotateBlock(data, data.pos().method_46558().method_43206(data.dir(), 0.5D), RotationType.BlockPlace, "placing")) {
               if (!(Boolean)this.pauseOffGround.get() || BlackOut.mc.field_1724.method_24828()) {
                  ...
                  case Instant:
                     this.sendPacket(new LookAndOnGround(this.placeDir.method_10144(), Managers.ROTATION.nextPitch, Managers.PACKET.isOnGround()));
                     break;
                  case Manager:
                  ...
                  if (hand != null || (switched = ((SwitchMode)this.switchMode.get()).swap(result.slot()))) {
                     ...
                     Item var7 = stack.method_7909();
                     if (var7 instanceof BedItem) {
                        BedItem bedItem = (BedItem)var7;
                        BlockState feetState = (BlockState)((BlockState)bedItem.method_7711().method_9564().method_11657(BedBlock.field_9967, BedPart.field_12557)).method_11657(HorizontalFacingBlock.field_11177, this.placeDir);
                        BlockState headState = (BlockState)((BlockState)bedItem.method_7711().method_9564().method_11657(BedBlock.field_9967, BedPart.field_12560)).method_11657(HorizontalFacingBlock.field_11177, this.placeDir);
   ...
   private void updateExplode() {
      BlockState state = BlackOut.mc.field_1687.method_8320(this.explodePos);
      if (state.method_26204() instanceof BedBlock) {
   ...
   private void explode(BlockPos pos) {
      ...
      if (dir != null) {
         Vec3d placeVec = new Vec3d((double)pos.method_10263() + 0.5D, (double)pos.method_10264() + (Double)this.rotationHeight.get(), (double)pos.method_10260() + 0.5D);
         this.end("placing");
         if (!SettingUtils.shouldRotate(RotationType.Interact) || this.rotateBlock(pos, dir, placeVec, RotationType.Interact, 0.1D, "explode")) {
            ...
            Direction direction = (Direction)state.method_11654(HorizontalFacingBlock.field_11177);
            BlockPos headPos;
            BlockPos feetPos;
            if (state.method_11654(BedBlock.field_9967) == BedPart.field_12560) {
               headPos = pos;
            ...
            BlackOut.mc.field_1687.method_8501(feetPos, Blocks.field_10124.method_9564());
            BlackOut.mc.field_1687.method_8501(headPos, Blocks.field_10124.method_9564());
            this.ignoreState.add(feetPos, 0.3D);
            this.ignoreState.add(headPos, 0.3D);
            this.interactBlock(Hand.field_5808, placeVec, dir, pos);
            ...
            if ((Boolean)this.explodeSwing.get()) {
               this.clientSwing((SwingHand)this.explodeHand.get(), Hand.field_5808);
            }
      ...
      if (state.method_26204() instanceof BedBlock) {
         if (SettingUtils.getPlaceOnDirection(pos) != null) {
            if (SettingUtils.inPlaceRange(pos)) {
               this.calcDamage(state.method_11654(BedBlock.field_9967) == BedPart.field_12560 ? pos : pos.method_10093((Direction)state.method_11654(HorizontalFacingBlock.field_11177)));
            ...
            if (this.inRangeToEnemies(pos)) {
               ...
               if (this.placeDamageCheck()) {
                  ...
                  if (!(value <= this.calcValue)) {
                     Iterator var5 = Type.field_11062.iterator();
   ...
   private boolean validFloor(BlockPos pos) {
      return BlackOut.mc.field_1687.method_8320(pos).method_26204() instanceof BedBlock ? false : OLEPOSSUtils.solid2(pos);
   }

   private boolean validBlock(BlockPos pos) {
      Block block = BlackOut.mc.field_1687.method_8320(pos).method_26204();
      if (!this.validBlock(block) && !(block instanceof BedBlock)) {
         return false;
      } else {
         return !(Boolean)this.fireBlocking.get() || !(block instanceof FireBlock);
      }
   }
