...
public class Blocker extends Module {
   ...
   private final TimerList<Pair<BlockPos, Integer>> mining;
   ...
   public Blocker() {
      ...
      this.blocks = this.sgBlocks.bl("Blocks", "Blocks to use.", Blocks.field_10540);
   ...
   public void onReceive(PacketEvent.Receive.Pre event) {
      ...
      if (var3 instanceof BlockBreakingProgressS2CPacket) {
         BlockBreakingProgressS2CPacket p = (BlockBreakingProgressS2CPacket)var3;
      ...
      if (!(Boolean)this.pauseEat.get() || !BlackOut.mc.field_1724.method_6115()) {
         ...
         this.placePositions.stream().filter((pos) -> {
            return !EntityUtils.intersects(BoxUtils.get(pos), (entity) -> {
               return entity instanceof EndCrystalEntity && System.currentTimeMillis() - this.lastAttack > 100L;
            });
         }).forEach(this::place);
         if (this.switched) {
   ...
   private void addPlacePositions(Blocker.ProtectBlock p) {
      ...
      case 1:
         ...
         for(int var4 = 0; var4 < var9; ++var4) {
            Direction dir = var8[var4];
            if (p.type == 1) {
               if (!(Boolean)this.surroundSides.get() && dir.method_10166().method_10179() || !(Boolean)this.surroundTop.get() && dir == Direction.field_11036 || !(Boolean)this.surroundBottom.get() && dir == Direction.field_11033) {
                  continue;
               }
            } else if (dir == Direction.field_11036 || !(Boolean)this.surroundFloor.get() && dir.method_10166().method_10179() || !(Boolean)this.surroundFloorBottom.get() && dir == Direction.field_11033) {
               continue;
            }

            BlockPos pos = p.pos.method_10093(dir);
      ...
      case 2:
      case 3:
         BlockPos pos = p.pos.method_10084();
         if (!OLEPOSSUtils.replaceable(pos)) {
            return;
         }
   ...
   private void setBlock(BlockPos pos) {
      Item item = BlackOut.mc.field_1724.method_31548().method_5438(this.result.slot()).method_7909();
   ...
   private boolean shouldProtect(Blocker.ProtectBlock p) {
      ...
      switch(p.type) {
      case 1:
         if (OLEPOSSUtils.solid2(pos) && BlackOut.mc.field_1687.method_8320(pos).method_26204() != Blocks.field_9987) {
            break;
         }
      ...
      case 2:
      case 3:
         if (BlackOut.mc.field_1687.method_8320(p.pos).method_26204() != Blocks.field_10540) {
            return false;
         }

         if (!(BlackOut.mc.field_1687.method_8320(p.pos.method_10084()).method_26204() instanceof AirBlock)) {
            return false;
         }

         if (SettingUtils.oldCrystals() && !(BlackOut.mc.field_1687.method_8320(p.pos.method_10086(2)).method_26204() instanceof AirBlock)) {
            return false;
         }
      }

      return !this.mining.contains((timer) -> {
         return ((BlockPos)((Pair)timer.value).method_15442()).equals(p.pos) && (double)(System.currentTimeMillis() - timer.startTime) >= (Double)this.mineTime.get() * 1000.0D;
   ...
   private void updateBlocks() {
      ...
      if (!(Boolean)this.onlyHole.get() || HoleUtils.inHole(BlackOut.mc.field_1724.method_24515())) {
         BlockPos e = BlockPos.method_49637(BlackOut.mc.field_1724.method_23317(), BlackOut.mc.field_1724.method_5829().field_1325, BlackOut.mc.field_1724.method_23321());
         BlockPos pos = new BlockPos(BlackOut.mc.field_1724.method_31477(), (int)Math.round(BlackOut.mc.field_1724.method_23318()), BlackOut.mc.field_1724.method_31479());
         int[] size = new int[4];
            ...
            for(int x = -2; x <= 2; ++x) {
               for(int y = -2; y <= 2; ++y) {
                  for(int z = -2; z <= 2; ++z) {
                     ...
                     if (BlackOut.mc.field_1687.method_8320(pos).method_26204() instanceof AirBlock && (!SettingUtils.oldCrystals() || BlackOut.mc.field_1687.method_8320(pos.method_10084()).method_26204() instanceof AirBlock) && !EntityUtils.intersects(BoxUtils.crystalSpawnBox(pos), (entity) -> {
                        return !(entity instanceof ExperienceOrbEntity) && !(entity instanceof ExperienceBottleEntity);
   ...
   public static record Render(BlockPos pos, long time) {
      public Render(BlockPos pos, long time) {
         this.pos = pos;
         this.time = time;
      }
   ...
   private static record ProtectBlock(BlockPos pos, int type) {
      private ProtectBlock(BlockPos pos, int type) {
         this.pos = pos;
         this.type = type;
      }
