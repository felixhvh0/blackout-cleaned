...
public class BurrowRewrite extends Module {
   ...
   public BurrowRewrite() {
      ...
      this.tick = 0;
      this.startPos = Vec3d.field_1353;
      this.prevFinish = 0L;
      this.modifiedTimer = false;
      this.predicate = (stack) -> {
         Item patt4262$temp = stack.method_7909();
         boolean var10000;
         if (patt4262$temp instanceof BlockItem) {
            BlockItem blockItem = (BlockItem)patt4262$temp;
      ...
      if (!OLEPOSSUtils.inside(BlackOut.mc.field_1724, BlackOut.mc.field_1724.method_5829().method_1012(0.0D, (double)this.calcY(), 0.0D)) && System.currentTimeMillis() - this.prevFinish >= 1000L && !this.notFound()) {
         ...
         if (BlackOut.mc.field_1724.method_24828()) {
            if (this.mode.get() == BurrowRewrite.BurrowMode.Cancel) {
               this.shouldCancel = true;
               this.sendPacket(new PositionAndOnGround(BlackOut.mc.field_1724.method_23317(), 1337.0D, BlackOut.mc.field_1724.method_23321(), false));
            }

            if ((Boolean)this.instant.get()) {
               Vec3d prevPos = BlackOut.mc.field_1724.method_19538();
   ...
   private PlaceData preInstant(Vec3d prevPos) {
      PlaceData data = SettingUtils.getPlaceData(BlockPos.method_49638(prevPos));
      if (!data.valid()) {
         return null;
      } else {
   ...
   private void rubberband() {
      ...
      for(int i = 0; i < (Integer)this.packets.get(); ++i) {
         this.sendPacket(new PositionAndOnGround(x, y, z, false));
      }
      ...
      if ((Boolean)this.smooth.get()) {
         ...
         if ((Boolean)this.syncPacket.get()) {
            Managers.PACKET.sendInstantly(new Full(this.startPos.method_10216(), this.startPos.method_10214(), this.startPos.method_10215(), Managers.ROTATION.prevYaw, Managers.ROTATION.prevPitch, false));
         }
   ...
   private PlaceData getPlaceData() {
      return SettingUtils.getPlaceData(BlockPos.method_49638(this.startPos));
   }
