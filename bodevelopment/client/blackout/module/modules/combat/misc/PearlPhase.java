...
public class PearlPhase extends Module {
   ...
   public void onRender(RenderEvent.World.Pre event) {
      if (BlackOut.mc.field_1724 != null && BlackOut.mc.field_1687 != null) {
         Hand hand = OLEPOSSUtils.getHand(Items.field_8634);
         FindResult findResult = ((SwitchMode)this.switchMode.get()).find(Items.field_8634);
            ...
            if (!(Boolean)this.ccBypass.get() || this.cc() || this.placed) {
               ...
               if (switched || ((SwitchMode)this.switchMode.get()).swap(findResult.slot())) {
                  if (this.rotationMode.get() == ObsidianModule.RotationMode.Packet) {
                     this.sendPacket(new LookAndOnGround((float)this.getYaw(), (float)(Integer)this.pitch.get(), Managers.PACKET.isOnGround()));
                  }
   ...
   private boolean cc() {
      ...
      if (!(result = ((SwitchMode)this.ccSwitchMode.get()).find((stack) -> {
         return stack.method_7909() instanceof BlockItem;
      })).wasFound()) {
         this.disable("no CC blocks found");
         return false;
      } else {
         BlockPos pos = BlackOut.mc.field_1724.method_24515();
         if (SettingUtils.shouldRotate(RotationType.BlockPlace)) {
            switch((ObsidianModule.RotationMode)this.rotationMode.get()) {
            case Normal:
               if (!this.rotateBlock(pos.method_10074(), Direction.field_11036, RotationType.BlockPlace, "placing")) {
                  return false;
               }
               break;
            case Instant:
               if (!this.rotateBlock(pos.method_10074(), Direction.field_11036, RotationType.InstantBlockPlace, "placing")) {
                  return false;
               }
            }
         }

         Hand hand = OLEPOSSUtils.getHand((stack) -> {
            return stack.method_7909() instanceof BlockItem;
         });
         if (hand == null && !((SwitchMode)this.ccSwitchMode.get()).swap(result.slot())) {
            ...
            if (SettingUtils.shouldRotate(RotationType.BlockPlace) && this.rotationMode.get() == ObsidianModule.RotationMode.Packet) {
               this.sendPacket(new LookAndOnGround((float)RotationUtils.getYaw(pos.method_46558()), (float)RotationUtils.getPitch(BlackOut.mc.field_1724.method_33571(), pos.method_46558()), Managers.PACKET.isOnGround()));
            }

            this.placeBlock(hand == null ? Hand.field_5808 : hand, pos.method_10074().method_46558(), Direction.field_11036, pos.method_10074());
            if (SettingUtils.shouldRotate(RotationType.BlockPlace)) {
               this.end("placing");
            }
   ...
   private int getYaw() {
      return (int)Math.round(RotationUtils.getYaw(new Vec3d(Math.floor(BlackOut.mc.field_1724.method_23317()) + 0.5D, 0.0D, Math.floor(BlackOut.mc.field_1724.method_23321()) + 0.5D))) + 180;
   }
}
