...
public class AntiAim extends Module {
   ...
   public void onRender(RenderEvent.World.Pre event) {
      ...
      case Enemy:
         ...
         if (target != null) {
            yaw = RotationUtils.getYaw((Entity)target);
         } else {
      ...
      case CSGO:
         if ((double)System.currentTimeMillis() > (double)this.prevCsgo + 1000.0D / (Double)this.csgoSpeed.get()) {
            this.csgoYaw = MathHelper.method_16436(Math.random(), (Double)this.csgoYawMin.get(), (Double)this.csgoYawMax.get());
            this.csgoPitch = MathHelper.method_16436(Math.random(), (Double)this.csgoPitchMin.get(), (Double)this.csgoPitchMax.get());
            ...
            do {
               do {
                  do {
                     ...
                     player = (AbstractClientPlayerEntity)var6.next();
   ...
   private AntiAim.IgnoreMode getIgnore(Item item) {
      if (item == Items.field_8287) {
         return (AntiAim.IgnoreMode)this.iExp.get();
      } else if (item == Items.field_8634) {
         return (AntiAim.IgnoreMode)this.iPearl.get();
      } else if (item != Items.field_8102 && item != Items.field_8399) {
         return item instanceof ThrowablePotionItem ? (AntiAim.IgnoreMode)this.iPotion.get() : AntiAim.IgnoreMode.Disabled;
      } else {
