...
public class RotationSettings extends SettingsModule {
   ...
   public RotationSettings() {
      ...
      this.sinceRotated = 0;
      this.vec = new Vec3d(0.0D, 0.0D, 0.0D);
      INSTANCE = this;
   }
   ...
   public boolean blockRotationCheck(BlockPos pos, Direction dir, float yaw, float pitch, RotationType type) {
      ...
      case StrictRaytrace:
         ...
         if (result.method_17783() == Type.field_1332 && result.method_17777().equals(pos)) {
            return true;
         }
      ...
      case DirectionStrict:
         ...
         ((IRaycastContext)DamageUtils.raycastContext).blackout_Client$set(BlackOut.mc.field_1724.method_33571(), end);
         BlockHitResult result = DamageUtils.raycast(DamageUtils.raycastContext, false);
         if (result.method_17783() == Type.field_1332 && result.method_17777().equals(pos) && result.method_17780() == dir) {
            return true;
         }
   ...
   private Box expandedBox(BlockPos pos, RotationType type) {
      ...
      double hz = var10000;
      return new Box((double)pos.method_10263() - hz, (double)pos.method_10264() - down, (double)pos.method_10260() - hz, (double)(pos.method_10263() + 1) + hz, (double)(pos.method_10264() + 1) + up, (double)(pos.method_10260() + 1) + hz);
   }
   ...
   public Rotation applyStep(Rotation rotation, RotationType type, boolean rotated) {
      ...
      op = (float)MathHelper.method_15350((double)op + (Math.random() - 0.5D) * o, -90.0D, 90.0D);
      ...
      if (type.instant) {
         ...
         case Smooth:
            ...
            if ((Boolean)this.pauseRotated.get() && this.sinceRotated < (Integer)this.delay.get()) {
               ...
               speed = MathHelper.method_15390((Double)this.minSmoothSpeed.get(), (Double)this.maxSmoothSpeed.get(), MathHelper.method_15370(total, (Double)this.minSmoothAngle.get(), (Double)this.maxSmoothAngle.get()));
            }
   ...
   private double getBalanceSpeed() {
      ...
      if (length <= 1) {
         ...
         return MathHelper.method_15350((Double)this.averageSpeed.get() * (double)length - total, 0.0D, (Double)this.maxSpeed.get());
      }
   }
   ...
   public boolean attackRotationCheck(Box box, float yaw, float pitch) {
      if (box.method_1006(BlackOut.mc.field_1724.method_33571())) {
         ...
         case Raytrace:
            ...
            break;
         case StrictRaytrace:
            Vec3d end = RotationUtils.rotationVec((double)yaw, (double)pitch, BlackOut.mc.field_1724.method_33571(), 7.0D);
            Optional<Vec3d> pos = box.method_992(BlackOut.mc.field_1724.method_33571(), end);
            if (pos.isEmpty()) {
               return false;
            }

            ((IRaycastContext)DamageUtils.raycastContext).blackout_Client$set(BlackOut.mc.field_1724.method_33571(), (Vec3d)pos.get());
            boolean visible = DamageUtils.raycast(DamageUtils.raycastContext, false).method_17783() == Type.field_1333;
            if (visible) {
               return true;
            }
   ...
   public Vec3d getRotationVec(BlockPos pos, Direction dir, Vec3d vec, RotationType type) {
      ...
      if (mode != RotationSettings.BlockRotationCheckMode.Raytrace && mode != RotationSettings.BlockRotationCheckMode.Angle) {
         if (mode == RotationSettings.BlockRotationCheckMode.DirectionRaytrace) {
            ...
            VoxelShape shape = state.method_26218(BlackOut.mc.field_1687, pos);
            Box box;
            if (shape.method_1110()) {
               box = new Box(0.0D, 0.0D, 0.0D, 1.0D, 1.0D, 1.0D);
            } else {
   ...
   public Vec3d getRaytraceRotationVec(Box box, Vec3d vec) {
      Vec3d v = new Vec3d(0.0D, 0.0D, 0.0D);
      double cd = 100.0D;
      double ox = MathHelper.method_15370(vec.field_1352, box.field_1323, box.field_1320);
      double oy = MathHelper.method_15370(vec.field_1351, box.field_1322, box.field_1325);
      double oz = MathHelper.method_15370(vec.field_1350, box.field_1321, box.field_1324);
            ...
            for(double z = 0.1D; z <= 0.9D; z += 0.1D) {
               ...
               if (!(distance >= SettingUtils.getAttackRange())) {
                  if (distance > SettingUtils.getAttackWallsRange()) {
                     ...
                     BlockHitResult result = DamageUtils.raycast(DamageUtils.raycastContext, false);
                     if (result.method_17783() != Type.field_1333) {
                        continue;
                     }
                  }
   ...
   public static enum PacketRotationMode {
      ...
      Basic((yaw, pitch) -> {
         ...
         Managers.PACKET.sendPacket(new Full(pos.method_10216(), pos.method_10214(), pos.method_10215(), yaw, pitch, Managers.PACKET.isOnGround()));
      }),
      Double((yaw, pitch) -> {
         Vec3d pos = Managers.PACKET.pos;
      ...
      Return((yaw, pitch) -> {
         ...
         Vec3d pos = Managers.PACKET.pos;
         Managers.PACKET.sendPacket(new Full(pos.method_10216(), pos.method_10214(), pos.method_10215(), yaw, pitch, Managers.PACKET.isOnGround()));
         Managers.PACKET.sendPacket(new Full(pos.method_10216(), pos.method_10214(), pos.method_10215(), prevYaw, prevPitch, Managers.PACKET.isOnGround()));
      });
