...
public class Scaffold extends MoveUpdateModule {
   ...
   private final TimerList<BlockPos> render;
   ...
   public Scaffold() {
      ...
      this.blocks = this.sgPlacing.bl("Blocks", "Blocks to use.", Blocks.field_10540, Blocks.field_22423, Blocks.field_22108, Blocks.field_10340, Blocks.field_10161, Blocks.field_10375);
      ...
      this.movement = Vec3d.field_1353;
      this.result = null;
      this.switched = false;
      this.hand = null;
   ...
   public void onRender(RenderEvent.World.Post event) {
      ...
      case Placed:
         this.render.forEach((timer) -> {
            double progress = 1.0D - MathHelper.method_15350(MathHelper.method_15370((double)System.currentTimeMillis(), (double)timer.startTime, (double)timer.endTime), 0.0D, 1.0D);
            this.rendering.render(BoxUtils.get((BlockPos)timer.value), (float)progress, 1.0F);
         });
         break;
      case NotPlaced:
   ...
   private void updateBlocks(Vec3d motion) {
      ...
      Box box = BlackOut.mc.field_1724.method_5829();
      double x;
      if (this.shouldKeepY()) {
   ...
   private boolean addBlocks2(Box box, Direction[] directions, int b) {
      BlockPos feetPos = BlockPos.method_49638(BoxUtils.feet(box).method_1031(0.0D, -0.5D, 0.0D));
      if (OLEPOSSUtils.replaceable(feetPos) && !this.positions.contains(feetPos) && !this.intersects(feetPos)) {
         if (b < 1 && this.validSupport(feetPos, true)) {
            ...
            for(int i = 0; (double)i < Math.pow((double)l, (double)b); ++i) {
               for(int j = 0; j < b; ++j) {
                  Direction dir = directions[i / (int)Math.pow((double)l, (double)j) % l];
                  drr[b - j - 1] = dir;
               }
   ...
   private Direction[] getDirections(Vec3d motion) {
      double dir = RotationUtils.getYaw(new Vec3d(0.0D, 0.0D, 0.0D), motion, 0.0D);
      Direction moveDir = Direction.method_10150(dir);
      return new Direction[]{moveDir.method_10153(), moveDir, moveDir.method_10160(), moveDir.method_10170(), Direction.field_11036, Direction.field_11033};
   }
         ...
         if (this.result.amount() > 0) {
            if (this.blocksLeft > 0) {
               ...
               if (SettingUtils.shouldRotate(RotationType.BlockPlace)) {
                  ...
                  Vec3d vec = this.getRotationVec(data.pos(), (double)rotation.pitch());
                  if (vec != null) {
   ...
   private Vec3d getRotationVec(BlockPos pos, double pitch) {
      ...
      if (this.movement.method_37268() > 0.0D) {
         ...
         case SemiLocked:
            yaw = (double)(Math.round(RotationUtils.getYaw(this.movement, Vec3d.field_1353, 0.0D) / 45.0D) * 45L);
            break;
         case Locked:
            yaw = (double)(Math.round(RotationUtils.getYaw(this.movement, Vec3d.field_1353, 0.0D) / 90.0D) * 90L);
            break;
         case Back:
            yaw = RotationUtils.getYaw(this.movement, Vec3d.field_1353, 0.0D);
            break;
         default:
            return null;
         }
   ...
   private Hand getHand() {
      if (this.valid(Managers.PACKET.getStack())) {
         ...
         return this.valid(BlackOut.mc.field_1724.method_6079()) ? Hand.field_5810 : null;
      }
   }
