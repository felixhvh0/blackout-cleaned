...
public class TargetStrafe extends Module {
   ...
   private void calc(boolean right, double movement) {
      ...
      for(double delta = -1.0D; delta <= 1.0D; delta += 0.01D) {
         ...
         if (!(diff >= this.closest)) {
            ...
            if (yaw != null) {
               Vec3d vec = (new Vec3d(d * Math.cos(yaw), 0.0D, d * Math.sin(yaw))).method_1019(BlackOut.mc.field_1724.method_19538());
               double width = 0.3D;
               double height = 1.8D;
               Box box = new Box(vec.method_10216() - width, vec.method_10214(), vec.method_10215() - width, vec.method_10216() + width, vec.method_10214() + height, vec.method_10215() + width);
   ...
   private Double doTheMathing(double movement, double preferred, double distance, boolean reversed) {
      ...
      return Double.isNaN(angle) ? null : Math.toRadians(RotationUtils.getYaw((Entity)this.target)) + Math.abs(angle) * (double)(reversed ? 1 : -1) + 1.5707963705062866D;
   }
