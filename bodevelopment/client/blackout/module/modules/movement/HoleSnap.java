...
public class HoleSnap extends Module {
   ...
   public void onPacket(PacketEvent.Receive.Pre event) {
      if (event.packet instanceof PlayerPositionLookS2CPacket && (Integer)this.maxRubberbands.get() > 0 && ++this.rubberbands >= (Integer)this.maxRubberbands.get() && (Integer)this.maxRubberbands.get() > 0) {
   ...
   private boolean singleBlocked() {
      if (!(Boolean)this.singleTarget.get()) {
         ...
         BlockPos[] var1 = this.singleHole.positions;
   ...
   private Hole findHole() {
      ...
      for(int x = -r; x <= r; ++x) {
         for(int y = (int)(-Math.ceil((Double)this.downRange.get())); y < 1; ++y) {
            for(int z = -r; z <= r; ++z) {
               BlockPos pos = OLEPOSSUtils.roundedPos().method_10069(x, y, z);
