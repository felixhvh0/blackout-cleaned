...
public class DoubleSetting extends Setting<Double> {
   ...
   public float render() {
      if (this.moving) {
         ...
         float val = (float)MathHelper.method_16436((double)this.sliderPos, this.min, this.max);
         this.setValue((double)Math.round((double)val / this.step) * this.step);
      } else {
         this.sliderPos = (float)MathHelper.method_15350(MathHelper.method_15370((Double)this.get(), this.min, this.max), 0.0D, 1.0D);
      }
