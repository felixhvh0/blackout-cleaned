   ...
   public String execute(String[] args) {
      if (args.length > 0) {
         ...
         if (module == null) {
            ...
            if (similar != null) {
               return String.format("%s couldn't find %s%s%s from modules, did you mean %s", Formatting.field_1061, Formatting.field_1079, built, Formatting.field_1061, this.moduleNameString(similar));
            } else {
               return String.format("%s couldn't find %s%s%s from modules", Formatting.field_1061, Formatting.field_1079, built, Formatting.field_1061);
            }
         } else if (module.toggleable()) {
            module.enable();
            String displayName = module.getDisplayName();
            String var10000 = this.color;
            return var10000 + this.lowerCase + "d " + Formatting.field_1068 + this.moduleNameString(module);
         } else {
            return String.format("%s%s%s is not toggleable", Formatting.field_1079, this.moduleNameString(module), Formatting.field_1061);
         }
      } else {
         return this.format;
      }
