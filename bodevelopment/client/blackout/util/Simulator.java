...
public class Simulator {
   ...
   private static void postMove(SimulationContext ctx) {
      if (ctx.inFluid() && !ctx.inFluidOriginal()) {
         ctx.motionY = MathHelper.method_15350(ctx.motionY, -0.0784D, 0.13D);
      }
   ...
   private static void handleCollisions(SimulationContext ctx) {
      ...
      Vec3d collidedMovement = movement.method_1027() == 0.0D ? movement : ctx.collide(movement, ctx.box);
      ...
      if ((ctx.onGround || collidingWithFloor) && collidedHorizontally) {
         ...
         Vec3d vec3 = ctx.collide(new Vec3d(0.0D, ctx.step, 0.0D), ctx.box.method_1012(ctx.motionX, 0.0D, ctx.motionZ));
         Vec3d vec;
         if (vec3.field_1351 < ctx.step) {
            vec = ctx.collide(new Vec3d(movement.field_1352, 0.0D, movement.field_1350), ctx.box.method_997(vec3)).method_1019(vec3);
         ...
         if (vec2.method_37268() > collidedMovement.method_37268()) {
            vec = vec2.method_1019(ctx.collide(new Vec3d(0.0D, -vec2.field_1351 + movement.field_1351, 0.0D), ctx.box.method_997(vec2)));
            ctx.move(vec);
