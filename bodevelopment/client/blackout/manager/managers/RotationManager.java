...
public class RotationManager extends Manager {
   ...
   public void onRender(RenderEvent.World.Pre event) {
      if (this.rotatingYaw == RotationManager.RotatePhase.Rotating && SettingUtils.shouldVanillaRotate()) {
         BlackOut.mc.field_1724.method_36456(MathHelper.method_16439(event.tickDelta, this.prevRenderYaw, this.renderYaw));
      }

      if (this.rotatingPitch == RotationManager.RotatePhase.Rotating && SettingUtils.shouldVanillaRotate()) {
         BlackOut.mc.field_1724.method_36457(MathHelper.method_16439(event.tickDelta, this.prevRenderPitch, this.renderPitch));
      }
   ...
   public int updateMove(float yaw, boolean move) {
      this.moveYaw = yaw;
      this.move = move;
      int offset = Math.round(MathHelper.method_15393(yaw - this.moveLookYaw) / 45.0F);
