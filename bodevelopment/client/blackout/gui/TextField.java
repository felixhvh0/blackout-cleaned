...
public class TextField {
   ...
   public void render(MatrixStack stack, float scale, double mx, double my, float x, float y, float width, float height, float radius, float shadow, Color textColor, Color bgColor) {
      this.width = width;
      this.height = height;
      this.mx = mx - (double)x;
      this.my = my - (double)y;
   ...
   public void type(int key, boolean state) {
      if (state && (key >= 39 && key <= 92 || key == 262 || key == 263 || key == 259 || key == 32)) {
         ...
         case 262:
            this.lastType = System.currentTimeMillis();
            this.typingIndex = MathHelper.method_15340(this.typingIndex + 1, 0, this.content.length());
            return;
         case 263:
            this.lastType = System.currentTimeMillis();
            this.typingIndex = MathHelper.method_15340(this.typingIndex - 1, 0, this.content.length());
   ...
   private void limitIndex() {
      this.typingIndex = MathHelper.method_15340(this.typingIndex, 0, this.content.length());
   }
